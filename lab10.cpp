...
int findImbalance (Node*bb, Node *&tp)
{
    if (bb==tp)
        return 0;
    else if (bb == bb->parent->left)
        bb->parent->bf--;
    else if (bb == bb->parent->right)
        bb->parent->bf++;
    if (bb->parent->bf > 1)
    {
        cout << "Imbalance at " << bb->parent->val << ", left subtree." << endl;
        return -1;
    }
    if (bb->parent->bf < -1)
    {
        cout << "Imbalance at " << bb->parent->val << ", right subtree." << endl;
        return -1;
    }

    return findImbalance(bb->parent, tp);
}
